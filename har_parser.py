...
class HARParser:
    ...
    def extract_requests_from_har(self, har_file_path: str,
        ...
        for i, entry in enumerate(entries):
            ...
            query_params = self._convert_bool_strings(query_params)
            ...
            if request.get('postData'):
                ...
                if content_type.startswith('multipart/form-data'):
                    ...
                    try:
                        ...
                        post_data = self._convert_bool_strings(post_data)
                        post_data = self._filter_invalid_params(post_data)
            ...
            if response.get('content', {}).get('text'):
                ...
                try:
                    ...
                    response_content = self._convert_bool_strings(response_content)
                except json.JSONDecodeError:
    ...
    def _convert_bool_strings(self, data: Any) -> Any:
        """
        递归转换字典/列表中的 true/false 字符串为布尔值，null 为 None
        """
        if isinstance(data, dict):
            return {k: self._convert_bool_strings(v) for k, v in data.items()}
        elif isinstance(data, list):
            return [self._convert_bool_strings(item) for item in data]
