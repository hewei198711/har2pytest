...
data = {
    ...
    "receiverFrom": 0,  # 接收者来源 0 未指定 1 全部用户 2导入 3自定义顾客群
    "receiverName": "",  # 导入名单文件名
    "receiverType": 0,  # 接收者类型  0 未指定 1服务中心 2商城
    "receiverUrl": "",  # 导入名单URL
    "sendTime": "",  # 发送时间
    "sendTimes": [],  # 发送时间
    "templateCode": "",  # 模板code
...
def _mgmt_msgadmin_handmade_saveOrUpdate(data=data, headers=headers):
    """
    新建或编辑手工消息
    /mgmt/msgadmin/handmade/saveOrUpdate
    ...
    - params: 需要替代的参数
    - planSceneId: 业务主题ID
    - receiverFrom: 接收者来源 0 未指定 1 全部用户 2导入 3自定义顾客群
    - receiverName: 导入名单文件名
    - receiverType: 接收者类型  0 未指定 1服务中心 2商城
    - receiverUrl: 导入名单URL
    - sendTime: 发送时间
    - sendTimes: 发送时间
    - templateCode: 模板code
