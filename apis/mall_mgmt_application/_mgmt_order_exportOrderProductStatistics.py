...
params = {
    ...
    "orderType": 0,  # 订单类型 1-正常订单 2->补报订单 3->调差订单 4->单位团购转分订单 5->85折转分订单 6->85折转分补报订单 7->预售订单 8->签约购订单 9->签约购转分订单 10->配件订单 11->体验中心自提订单 12->礼物订单 13->微信小店转分订单
    "orderTypeList": [],  # 订单类型 1-正常订单 2->补报订单 3->调差订单 4->单位团购转分订单 5->85折转分订单 6->85折转分补报订单 7->预售订单 8->签约购订单 9->签约购转分订单 10->配件订单 11->体验中心自提订单 12->礼物订单 13->微信小店转分订单
...
def _mgmt_order_exportOrderProductStatistics(params=params, headers=headers):
    """
    各系列产品业绩统计表导出
    /mgmt/order/exportOrderProductStatistics
    ...
    - orderNoList: 订单编号
    - orderStatusList: 订单状态 -5->待收礼 -4->活动押货待支付 -3->待支付定金 -2->待提交尾款 -1->待支付尾款 0->待审核 1->待支付 2->待发货 3->待收货 4->已取消 5->已退货 98->审核取消 99->已完成
    - orderType: 订单类型 1-正常订单 2->补报订单 3->调差订单 4->单位团购转分订单 5->85折转分订单 6->85折转分补报订单 7->预售订单 8->签约购订单 9->签约购转分订单 10->配件订单 11->体验中心自提订单 12->礼物订单 13->微信小店转分订单
    - orderTypeList: 订单类型 1-正常订单 2->补报订单 3->调差订单 4->单位团购转分订单 5->85折转分订单 6->85折转分补报订单 7->预售订单 8->签约购订单 9->签约购转分订单 10->配件订单 11->体验中心自提订单 12->礼物订单 13->微信小店转分订单
    ...
    url = "/mgmt/order/exportOrderProductStatistics"
